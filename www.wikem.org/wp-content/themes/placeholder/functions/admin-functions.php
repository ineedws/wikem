...
function woo_image($args) {
	...
	if ( empty($custom_field) && get_option('woo_auto_img') == 'true' && empty($img_link) && !is_singular() ) { 
	...
	if ( empty($custom_field) && empty($img_link) ) {
		...
		if ( $embed ) 
	    	$custom_field = woo_get_youtube_video_image($embed);
	}
...
// Get thumbnail from YouTube Embed code
if (!function_exists('woo_get_youtube_video_image')) { 
	function woo_get_youtube_video_image($youtube_code) { 
